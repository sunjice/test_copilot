...
export const useUserStore = defineStore("user", () => {
  ...
  async function login(loginRequest: LoginRequest): Promise<void> {
    const { accessToken, refreshToken, expiresIn } = await AuthAPI.login(loginRequest);
    rememberMe.value = loginRequest.rememberMe ?? false;
    AuthStorage.setTokens(accessToken, refreshToken, rememberMe.value, expiresIn);
  }
  ...
  async function loginByQrCode(ticket: string): Promise<void> {
    const { accessToken, refreshToken, expiresIn } = await AuthAPI.qrLogin(ticket);
    AuthStorage.setTokens(accessToken, refreshToken, false, expiresIn);
  }
  ...
  async function doRefreshToken(): Promise<void> {
    ...
    AuthStorage.setTokens(accessToken, newRefreshToken, AuthStorage.getRememberMe(), expiresIn);
  }
